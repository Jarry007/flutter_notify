  ...
  static Future<void> createNewNotification(
    ...
    await AwesomeNotifications().createNotification(
        content: NotificationContent(
            ...
            body: body ?? "这是一个弹窗消息",
            bigPicture:
                'https://boguanhuandianserver.oss-cn-beijing.aliyuncs.com/photo/3acb416a.jpg',
            // largeIcon:
            //     'https://storage.flutter-io.cn/cms-storage-bucket/0dbfcc7a59cd1cf16282.png',
...
Future<void> myNotifyScheduleInHours({
  ...
  var nowDate = DateTime.now().add(Duration(hours: hoursFromNow, seconds: 20));
  print('${nowDate.hour},${nowDate.minute},${nowDate.second}');
  await AwesomeNotifications().createNotification(
...
class _AppState extends State<MyApp> {
  ...
  initMqtt() async {
    ...
    client.subscribe("bgtestopic001/#", MqttQos.atLeastOnce);

    client.updates?.listen((List<MqttReceivedMessage<MqttMessage>> c) {
      final message = c[0].payload as MqttPublishMessage;
      final payload = const Utf8Decoder().convert(message.payload.message);

      print('Received message:$payload,${c[0].topic}');
      final topic = c[0].topic;
      if (topic == 'bgtestopic001/notify') {
        NotificationController.createNewNotification(
            title: topic, body: payload);
      }
    });
